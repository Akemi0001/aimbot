...
local UILibrary = loadstring(game:HttpGet("https://raw.githubusercontent.com/Akemi0001/aimbot/main/style.lua"))();
...
local DefaultSettings = {
    ...
    Esp = {
        ...
        Size = 16,
        RenderDistance = 9e9,
        Color = Color3.fromRGB(226, 19, 19),
...
local FullbrightEnabled = false -- Variável que armazena o estado do fullbright

local function ToggleFullbright()
    -- Lógica para ativar/desativar o fullbright no jogo
    if FullbrightEnabled then
        -- Código para ativar o fullbright
        game:GetService("Lighting").Ambient = Color3.new(1, 1, 1)
        game:GetService("Lighting").Brightness = 2
    else
        -- Código para desativar o fullbright
        game:GetService("Lighting").Ambient = Color3.new(0.5, 0.5, 0.5)
        game:GetService("Lighting").Brightness = 1
    end
end

local ShadowsEnabled = true -- Variável que armazena o estado das sombras

local function ToggleShadows()
    -- Lógica para ativar/desativar as sombras no jogo
    if ShadowsEnabled then
        -- Código para remover as sombras
        game:GetService("Lighting").GlobalShadows = false
    else
        -- Código para restaurar as sombras
        game:GetService("Lighting").GlobalShadows = true
    end
end

local AmbientColor = Color3.new(0.5, 0.5, 0.5) -- Cor padrão do ambiente

local function SetAmbientColor(Color)
    -- Define a cor do ambiente
    game:GetService("Lighting").Ambient = Color
end

local BrightnessValue = 1 -- Valor padrão do brilho

local function SetBrightness(Value)
    -- Define o valor do brilho
    game:GetService("Lighting").Brightness = Value
end

local DayTime = 12 -- Valor padrão do horário do dia

local function SetDayTime(Value)
    -- Define o horário do dia
    game:GetService("Lighting").TimeOfDay = Value
end

local RainRemoved = false -- Variável que armazena o estado de remoção da chuva

local function ToggleRainRemoval()
    -- Lógica para remover ou restaurar a chuva no jogo
    if RainRemoved then
        -- Código para restaurar a chuva
        game:GetService("Lighting").Atmosphere.Rain:Remove()
    else
        -- Código para remover a chuva
        local rain = Instance.new("Part")
        rain.Name = "Rain"
        -- Configurações para criar a aparência da chuva removida
        rain.Transparency = 1
        -- Outras configurações de aparência da chuva removida
        rain.Parent = game:GetService("Lighting").Atmosphere
    end
end

local FogRemoved = false -- Variável que armazena o estado de remoção da neblina

local function ToggleFogRemoval()
    -- Lógica para remover ou restaurar a neblina no jogo
    if FogRemoved then
        -- Código para restaurar a neblina
        game:GetService("Lighting").Atmosphere.Fog:Remove()
    else
        -- Código para remover a neblina
        local fog = Instance.new("Part")
        fog.Name = "Fog"
        -- Configurações para criar a aparência da neblina removida
        fog.Transparency = 1
        -- Outras configurações de aparência da neblina removida
        fog.Parent = game:GetService("Lighting").Atmosphere
    end
end

local ReflectionsRemoved = false -- Variável que armazena o estado de remoção das reflexões

local function ToggleReflectionsRemoval()
    -- Lógica para remover ou restaurar as reflexões no jogo
    if ReflectionsRemoved then
        -- Código para restaurar as reflexões
        game:GetService("Lighting").GlobalShadows = true
        -- Outras configurações para restaurar as reflexões
    else
        -- Código para remover as reflexões
        game:GetService("Lighting").GlobalShadows = false
        -- Outras configurações para remover as reflexões
    end
end

local DayTime = 12 -- Valor padrão do horário do dia
local LoopDayTimeEnabled = false -- Variável que armazena o estado de loop do horário do dia

local function ApplyDayTime()
    -- Lógica para aplicar o valor do horário do dia
    game:GetService("Lighting").TimeOfDay = tostring(DayTime)
end

local function LoopDayTime()
    -- Lógica para repetir o ciclo do horário do dia
    while LoopDayTimeEnabled do
        ApplyDayTime()
        wait(1) -- Aguarda um segundo antes de aplicar o valor novamente (ajuste conforme necessário)
        DayTime = DayTime + 0.1 -- Incrementa o valor do horário do dia
        if DayTime > 24 then
            DayTime = 0 -- Reinicia o valor do horário do dia quando chega a 24
        end
    end
end
...
local Window = MainUI:LoadWindow('<font color="#ff4f57">Akemi</font> Flow', UDim2.fromOffset(950, 600));
local Aimbot = Window.NewPage("Aimbot");
local ESP = Window.NewPage("Esp");
local Visuals = Window.NewPage("Visuals");
local EspSettingsUI = ESP.NewSection("Esp");
local WorldVisuals = Visuals.NewSection("World");
local RemoverVisuals = Visuals.NewSection("Remover");
...
WorldVisuals.Toggle("Fullbright", FullbrightEnabled, function(Callback)
    FullbrightEnabled = Callback
    ToggleFullbright()
end)
WorldVisuals.Toggle("Remove Shadows", ShadowsEnabled, function(Callback)
    ShadowsEnabled = Callback
    ToggleShadows()
end)
WorldVisuals.Toggle("Loop DayTime", LoopDayTimeEnabled, function(Callback)
    LoopDayTimeEnabled = Callback
    if LoopDayTimeEnabled then
        LoopDayTime()
    end
end)
WorldVisuals.Slider("Brightness", {Min = 0, Max = 2, Default = BrightnessValue, Step = 0.1}, function(Callback)
    BrightnessValue = Callback
    SetBrightness(Callback)
end)
WorldVisuals.ColorPicker("Ambient Color", AmbientColor, function(Callback)
    AmbientColor = Callback
    SetAmbientColor(Callback)
end)
...
RemoverVisuals.Toggle("Remove Rain", RainRemoved, function(Callback)
    RainRemoved = Callback
    ToggleRainRemoval()
end)
RemoverVisuals.Toggle("Remove Fog", FogRemoved, function(Callback)
    FogRemoved = Callback
    ToggleFogRemoval()
end)
RemoverVisuals.Toggle("Remove Reflections", ReflectionsRemoved, function(Callback)
    ReflectionsRemoved = Callback
    ToggleReflectionsRemoval()
end)
